...
require 'fat_free_semantic_menu'
require 'mocha'

class FatFreeSemanticMenuTest < ActiveSupport::TestCase
  def test_menu_to_s
    assert_equal FatFreeSemanticMenu.new(nil) {}.to_s, '<ul class="menu"></ul>'
  end
  ...
  def test_nested_menu
    ...
    menu = FatFreeSemanticMenu.new(nil) do |root|
...
NESTED
  ...
  protected
    def default_menu
      FatFreeSemanticMenu.new nil, :class => 'mymenu' do |root|
        root.add 'title', 'link'
      end
    end
end
